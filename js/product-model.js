export function sanitizeProduct(docId, product) {
  ...
  const tailleUSA = readArray(["tailleUSA", "Taille USA", "sizesUSA", "sizes", "Sizes"]).filter(Boolean).map(String);
  ...
  const item = {
    ...
    tailleUSA: tailleUSA.length ? tailleUSA : legacySizes.length ? legacySizes : ["M"],
...
export function isOutOfStockGlobally(product) {
  return String(product.tag || "").toLowerCase().includes("out of stock")
