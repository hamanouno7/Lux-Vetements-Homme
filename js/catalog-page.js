...
const validCat = ["tshirts", "hoodies", "pants", "shoes", "accessories"];
const urlCat = new URLSearchParams(location.search).get("category");
let currentFilter =
  urlCat && (urlCat === "all" || validCat.includes(urlCat)) ? urlCat : "all";
...
function renderLoopGallery() {
  ...
  const loopItems = allProducts.filter((product) => validCat.includes(product.category));
